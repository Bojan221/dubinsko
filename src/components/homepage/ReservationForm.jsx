...
function ReservationForm() {
  ...
  return (
    ...
    [&::-webkit-datetime-edit-minute-field]:text-gray-200"
        ...
        <div>
          <button className="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold py-3.5 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg cursor-pointer flex justify-center items-center gap-3" onClick={()=> handleSend()}>
            Pošalji zahtjev <FaTelegramPlane />
          </button>
        </div>
        <span className="w-full text-center text-gray-200">Javicemo vam se radi potvrde termina.</span>
      </div>
    </div>
  );
}
